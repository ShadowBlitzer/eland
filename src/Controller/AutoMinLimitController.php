...
class AutoMinLimitController extends Controller
{
	public function form(Request $request, string $schema)
	{
		...
		if ($row)
		{
			...
			if (is_string($data['exclusive']))
			{
				$data['exclusive'] = explode(',', $data['exclusive']);
			}

			if (is_string($data['trans_exclusive']))
			{
				$data['trans_exclusive'] = explode(',', $data['trans_exclusive']);
			}
		}
		else
		{
